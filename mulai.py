...
def get_harga_khusus(kode):
    for harga, daftar_kode in HARGA_KHUSUS.items():
        if kode in daftar_kode:
            return harga
    return None
...
def konversi_harga(kode, nominal):
    # cek harga khusus
    if harga := get_harga_khusus(kode):
        return harga

    # pembulatan ke ribuan terdekat
    nominal_bulat = round(nominal / 1000) * 1000

    # profit berdasarkan range
...
def get_all_paket_ids(kategori):
    ...
    try:
        response = requests.get(
            sanity_url_query, headers=headers, params={"query": query}
        )
        if response.status_code == 200:
            ...
            print(
                f"Error saat mengambil data dari Sanity: {response.status_code}, {response.text}"
            )
            return []
...
def delete_paket_documents(ids):
    ...
    mutations = {"mutations": [{"delete": {"id": _id}} for _id in ids]}

    try:
        response = requests.post(
            sanity_url_mutate, headers=headers, data=json.dumps(mutations)
        )
        if response.status_code == 200 or response.status_code == 202:
            print(f"{len(ids)} dokumen 'paket' berhasil dihapus.")
        else:
            print(
                f"Error saat menghapus dokumen: {response.status_code}, {response.text}"
            )
    except Exception as e:
...
def scrap_from_url(source_urls, product):
    ...
    kategori = {
        ...
        product, "lainnya"
    )  # Sesuaikan kategori dengan skema
        ...
        for url in urls:
            ...
            try:
                ...
                for row in soup.select("table.hidden-xs tbody tr"):
                    ...
                    if any(
                        re.search(
                            rf"(?<!\d){re.escape(x)}(?!\d)", produk, re.IGNORECASE
                        )
                        for x in [
                            "4 Hari",
                            "6 Hari",
                        ...
                        continue
                    desc = (
                        row.select_one("td:nth-child(2) b")
                        .get("data-title", "")
                        .replace("\n", " ")
                        .strip()
                        or "tidak ada deskripsi"
                    )
    ...
    if data:
        ...
        try:
            ...
            if response.status_code == 200 or response.status_code == 202:
                ...
                print(
                    f"Jumlah data yang berhasil disimpan: {len(data)}"
                )  # Menampilkan jumlah data

            else:
                print(
                    f"Error saat menambahkan data: {response.status_code}, {response.text}"
                )
        except Exception as e:
...
paket = (
    input(
        "Pilih paket (paket-internet, aktivasi-voucher-internet, voucher-internet, pulsa, topup-game): "
    )
    .strip()
    .lower()
)
...
while True:
    if paket in pilih_product:
        ...
        print(
            "Paket tidak valid. Silakan pilih dari paket yang tersedia: paket-internet, aktivasi-voucher-internet, pulsa."
        )
        paket = (
            input("Pilih paket (paket-internet, aktivasi-voucher-internet, pulsa): ")
            .strip()
            .lower()
        )
